        ...
        <div className="flex flex-col md:flex-row gap-8 h-[calc(100vh-8rem)]">
            <div className="w-full md:w-auto flex flex-col gap-4">
                ...
                <div className="bg-card text-card-foreground p-4 rounded-lg border border-border hidden md:block shadow-lg">
                    <h4 className="font-semibold text-foreground mb-2">Resumen Semanal</h4>
                    <div className="text-sm text-muted-foreground">
                        <p>Turnos en la semana: <span className="font-bold text-foreground">{appointments.length}</span></p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 bg-card text-card-foreground shadow-lg p-6 rounded-lg border border-border flex flex-col">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h2 className="text-xl font-bold text-foreground">
                            {selectedDate ? (
                                `Semana del ${format(startOfWeek(selectedDate, { weekStartsOn: 1 }), "d")} al ${format(endOfWeek(selectedDate, { weekStartsOn: 1 }), "d 'de' MMMM", { locale: es })}`
                            ) : 'Seleccione una fecha'}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                            {loading ? 'Cargando turnos...' : `${appointments.length} turnos esta semana`}
                        </p>
                    </div>
                ...
                <div className="space-y-8 overflow-auto flex-1">
                    ...
                    {[0, 1, 2, 3, 4, 5, 6].map(dayOffset => {
                        const dayDate = addDays(startOfWeek(selectedDate || new Date(), { weekStartsOn: 1 }), dayOffset);
                        const dayAppointments = appointments.filter(apt => isSameDay(new Date(apt.start_time), dayDate));

                        if (dayAppointments.length === 0) return null;

                        return (
                            <div key={dayOffset} className="space-y-3">
                                <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground sticky top-0 bg-card py-2 z-10 border-b border-border/50">
